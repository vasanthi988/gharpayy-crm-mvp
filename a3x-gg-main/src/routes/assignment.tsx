...
// In-Memory Data Store (Database run avvakapothe test cheskodaniki)
let assignments: Assignment[] = [
  {
    id: '1',
    title: 'React Fundamentals',
    description: 'Complete basic components assignment',
    dueDate: '2026-04-01',
    status: 'pending'
  }
];
...
router.put('/:id', (req: Request, res: Response) => {
  try {
    ...
    res.status(200).json({ success: true, data: assignments[index] });
  } catch (error) {
...
router.delete('/:id', (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const exists = assignments.some((item) => item.id === id);
